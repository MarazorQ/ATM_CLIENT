...
const BasicDepositModal: FC<BasicModalProps> = ({
    open, 
    handleClose, 
    handleCloseAccept, 
    currency_list, 
      ...
      >
        ...
        >
            <FormControl fullWidth sx={{ marginBottom: 2 }}>
                <InputLabel id="demo-simple-select-label">Deposit plan</InputLabel>
                <Select
                ...
                >
                    { deposit_types_list.map((type: any) => (<MenuItem value={type.id}>{type.name}</MenuItem>))}
                </Select>
            </FormControl>
            <Box sx={{ marginBottom: 2 }}>
                {depType === ''? null : (depType == '1'? '-on 40 days - 0.02% per year - revocable' : "-on 40 days - 0.02% per year - irrevocable")}
            </Box>
            <FormControl fullWidth sx={{ marginBottom: 2 }}>
...
export default BasicDepositModal
